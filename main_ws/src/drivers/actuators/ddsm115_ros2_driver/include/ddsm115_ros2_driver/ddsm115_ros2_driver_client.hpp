...
class DDSM115DriverClient {
 public:
  // Constructor
  DDSM115DriverClient(FeedbackCallback feedback_callback = nullptr,
  ...
  // Low-level packet generation (without serial side-effects)
  std::vector<uint8_t> create_mode_command(uint8_t motor_id, ControlLoopModes mode);
  std::vector<uint8_t> create_velocity_command(uint8_t motor_id, double rpm, bool brake = false);
