...
"""
モーター実機動作シーケンス再現プログラム (位置・速度制御切り替え対応版)
================================================
...
def load_config(config_path: str) -> dict:
    ...
    try:
        ...
        motor_id = default_config["motor"]["id"]
        if isinstance(motor_id, str):
            if motor_id.lower().startswith("0x"):
                default_config["motor"]["id"] = int(motor_id, 16)
            else:
                default_config["motor"]["id"] = int(motor_id)
...
def build_mode_select_cmd(motor_addr: int, mode_name: str) -> bytes:
    ...
    return bytes([
        0x41, 0x54, 0x90, 0x07, 0xEB, motor_addr,
        0x08, 0x05, 0x70, 0x00, 0x00, mode_value, 0x00, 0x00, 0x00, 0x0D, 0x0A,
    ])
...
def build_enable_cmd(motor_addr: int) -> bytes:
    ...
    # ユーザー提供フレーム: 41 54 18 07 eb fc 08 00 00 00 00 00 00 00 00 0d 0a
    return bytes([
        0x41, 0x54, 0x18, 0x07, 0xEB, motor_addr,
...
def build_velocity_cmd(motor_addr: int, target_speed: float) -> bytes:
    ...
    # floatをリトルエンディアンで4バイトに変換
    val_bytes = struct.pack("<f", float(target_speed))
    cmd = bytes([
        0x41, 0x54, 0x90, 0x07, 0xEB, motor_addr,
        0x08, 0x0A, 0x70, 0x00, 0x00,
        val_bytes[0], val_bytes[1], val_bytes[2], val_bytes[3],
        0x0D, 0x0A
    ])
    return cmd
...
def main():
    print("=" * 70)
    print("  モーター実機動作プログラム (位置/速度制御対応版) 起動中...")
    print("=" * 70)
    ...
    print(f"  - モーター ID    : {hex(motor_id)} ({motor_id})")
    ...
    try:
        ...
        for i in range(3):
            send_command(ser, f"Set Mode #{i+1}", build_mode_select_cmd(motor_id, control_mode))
            time.sleep(0.05)
        time.sleep(0.5)
        ...
        if control_mode == "position":
            print("\n=== 3.5. 制限速度・制限電流設定 ===")
            for i in range(3):
                send_command(ser, f"Set Limit Speed #{i+1}", build_limit_spd_cmd(motor_id, target_speed))
                time.sleep(0.05)
            time.sleep(0.1)
            for i in range(3):
                send_command(ser, f"Set Limit Current #{i+1}", build_limit_cur_cmd(motor_id, limit_current))
                time.sleep(0.05)
            time.sleep(0.5)

        print("\n=== 4. モーター有効化 (Enable) ===")
        for i in range(3):
            send_command(ser, f"Enable #{i+1}", build_enable_cmd(motor_id))
            time.sleep(0.05)
        time.sleep(0.5)
        ...
        if control_mode == "position":
            send_command(ser, "Init Pos (0.0)", build_position_cmd(motor_id, 0.0))
        else:
            send_command(ser, "Init Stop (0.0)", build_velocity_cmd(motor_id, 0.0))
        time.sleep(0.1)

        # 駆動フェーズ
        if control_mode == "position":
            print(f"\n=== 5. 位置指令送信 ({target_position} rad に {duration}秒間保持) ===")
            drive_cmd = build_position_cmd(motor_id, target_position)
        else:
            print(f"\n=== 5. 速度指令送信 ({target_speed} rad/s で {duration}秒間回転) ===")
            drive_cmd = build_velocity_cmd(motor_id, target_speed)
        ...
        while time.monotonic() - t_start < duration:
            count += 1
            send_command(ser, f"Drive #{count}", drive_cmd)
            time.sleep(0.05)
        ...
        for i in range(5):
            send_command(ser, f"Stop #{i+1}", stop_cmd)
            time.sleep(0.05)
        ...
        for i in range(5):
            try:
                send_command(ser, f"Emergency Stop #{i+1}", stop_cmd)
            except Exception:
                pass
            time.sleep(0.05)
    finally:
