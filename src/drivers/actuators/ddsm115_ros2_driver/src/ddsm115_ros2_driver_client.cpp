...
std::vector<uint8_t> DDSM115DriverClient::create_velocity_command(uint8_t motor_id, double rpm,
                                                                  bool brake) {
  std::vector<uint8_t> data;
  data.reserve(10);
  data.push_back(motor_id);
  data.push_back(0x64);
  uint16_t val_u16 =
      std::bit_cast<uint16_t>(static_cast<int16_t>(std::clamp(std::round(rpm), -330.0, 330.0)));
...
void DDSM115DriverClient::send_mode_command(uint8_t motor_id, ControlLoopModes mode) {
  ...
  std::stringstream ss;
  ss << "Sent mode command to motor " << static_cast<int>(motor_id) << ": mode "
...
bool DDSM115DriverClient::send_current_command(uint8_t motor_id, double current) {
  ...
  uint16_t val_u16 = std::bit_cast<uint16_t>(
      static_cast<int16_t>(std::clamp(std::round(current * (32767.0 / 8.0)), -32767.0, 32767.0)));
  data.push_back(static_cast<uint8_t>((val_u16 >> 8) & 0xFF));
  data.push_back(static_cast<uint8_t>(val_u16 & 0xFF));
  data.push_back(0x00);
  data.push_back(0x00);
  ...
  std::stringstream ss;
  ss << "Current command for motor " << static_cast<int>(motor_id) << ": current=" << current
     << ", val_u16=" << val_u16;
  log(LogLevel::DEBUG, ss.str());
...
bool DDSM115DriverClient::send_position_command(uint8_t motor_id, double position) {
  ...
  data.push_back(motor_id);
  data.push_back(0x64);
  uint16_t val_u16 = std::bit_cast<uint16_t>(static_cast<int16_t>(
      std::clamp(std::round((position - 180.0) * (32767.0 / 180.0)), -32767.0, 32767.0)));
  data.push_back(static_cast<uint8_t>((val_u16 >> 8) & 0xFF));
  data.push_back(static_cast<uint8_t>(val_u16 & 0xFF));
  data.push_back(0x00);
  data.push_back(0x00);
  ...
  std::stringstream ss;
  ss << "Position command for motor " << static_cast<int>(motor_id) << ": position=" << position
     << ", val_u16=" << val_u16;
  log(LogLevel::DEBUG, ss.str());
...
bool DDSM115DriverClient::send_rotate_command(std::vector<uint8_t> &data, uint8_t motor_id) {
  try {
    ...
    boost::asio::write(serial_port_, boost::asio::buffer(data, data.size()));
    std::stringstream ss;
    ss << "Sent command to motor " << static_cast<int>(motor_id) << ", mode "
