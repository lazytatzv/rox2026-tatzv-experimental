...
ament_uncrustify --reformat main_ws/src || true

# 2. Python Formatting (Black)
echo "[STYLE] Running Black (Python) reformat..."
black main_ws/src --line-length 100 --quiet || true
...
find main_ws/src -type f \( -name "*.cpp" -o -name "*.hpp" -o -name "*.py" -o -name "*.xml" -o -name "*.yaml" \) -exec sed -i 's/[[:space:]]*$//' {} +
