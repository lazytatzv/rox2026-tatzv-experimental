...
## 7. 開発環境と実行環境の分離 (Dev/Prod)

最強のプロフェッショナルは、ビルドツールを含む重いイメージと、実行バイナリのみの軽いイメージを使い分けます。本プロジェクトでは **Docker マルチステージビルド** によりこれを実現しています。

### 開発環境 (Dev Mode)
- **特徴**: コンパイラ、デバッガ、IDEサポート、ソースコードのライブマウントが含まれます。
- **用途**: コードの記述、デバッグ、単体テスト。
- **コマンド**: `make image` / `make up` (デフォルト)

### 実行環境 (Prod Mode)
- **特徴**: ビルドツールやソースコードを含まず、コンパイル済みのバイナリ (`install/` フォルダ) のみが含まれる超軽量イメージです。
- **用途**: ロボット実機へのデプロイ、最終動作確認、CI/CD。
- **コマンド**: `make prod-image` / `make prod-up`

### なぜ分けるのか？
1. **スピード**: Prodイメージはサイズが小さいため、ロボットへの転送や起動が劇的に速くなります。
2. **信頼性**: 「ビルド済みの成果物」をそのまま動かすため、開発環境の汚れに左右されない安定した動作が保証されます。
3. **セキュリティ**: 不要なツールやソースコードを実機に置かないため、堅牢性が向上します。
